...
function SFMC() {
    this.etClient = new ET_Client( config.clientId, config.clientSecret, config.stack );
}
...
SFMC.prototype.listAttributeSetDefinitions = function (callback) {
    var endpoint = "/contacts/v1/attributeSetDefinitions";
    this.etClient.RestClient.get(endpoint, function(err, response) {
        if (err) {
			callback(err);
		} 
...
SFMC.prototype.listAttributeSetDefinition = function (name, callback) {
    var endpoint = "/contacts/v1/attributeSetDefinitions/name:" + name;
        ...
        if (err) {
            ...
            if ( item.attributes ) {
                ...
                for( var i = 0; i < len; i++ ) {
                    ...
                    res.id = attr.id;
                    res.key = attr.key;
                    res.name = attr.name.value;
                    result.attributes.push(res);
                }
            }
